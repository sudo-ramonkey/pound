...
/*TODO: Change colors to fit a design*/
...
void editorSave()
{
    ...
    int fd = open(E.filename, O_RDWR | O_CREAT, 0644);//0644 permisos para el texto en UNIX
    if(fd != -1) {
...
/* TODO: Improve editorFind() so it uses regex searches to improve search efficiency */
...
{
    abAppend(ab, "\x1b[7m", 4);
    // abAppend(ab, "\x1b[32m", 7);
    char status[80], rstatus[80];
    // %.20s
    // %s
    int len = snprintf(status, sizeof(status), "%.20s",
        E.filename ? E.filename : "[No filename]", E.dirty ? "(modified)" : "");
